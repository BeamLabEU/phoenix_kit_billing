defmodule PhoenixKitBilling.MigrationsMoneySafetyTest do
  ...
  back. `phoenix_kit_invoices` and `phoenix_kit_transactions` are V4's own
  adopted tables; `phoenix_kit_orders` is V3's (and V4's); the config
  table is V1's — so this exercises the survival guarantee across the
  whole chain, not just its newest version.
  ...
  test "the rollback still does its one real job: the marker is cleared" do
    Repo.query!("COMMENT ON TABLE phoenix_kit_payment_provider_configs IS 'pkb_schema:4'")
