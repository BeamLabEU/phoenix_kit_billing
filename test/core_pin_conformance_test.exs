defmodule PhoenixKitBilling.CorePinConformanceTest do
  ...
  The floor is 2.26: `<.decimal_input>` and `PhoenixKit.Utils.Number.parse_decimal/2`
  first shipped in core 2.26.0, and five LiveViews import the component at compile
  ...
  @must_reject ["1.7.189", "1.7.236", "1.9.4", "2.0.0", "2.25.0", "3.0.0"]
    ...
    for version <- @must_admit do
      assert Version.match?(version, requirement),
             "`:phoenix_kit` requirement #{inspect(requirement)} rejects core #{version}. " <>
               ...
               "running this module alongside that core. Keep it a two-segment `~> 2.<floor>`."
    end
